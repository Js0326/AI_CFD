...
export function TestResultsPanel({ testResults }: TestResultsPanelProps) {
  ...
  const getTrendIndicator = (testType: string, value: number) => {
    ...
    const isLowerBetter =
      ...
      (testType === "multitasking" && value > 300)
  ...
  const getProgressValue = (testType: string, value: number) => {
    switch (testType) {
      case "typing":
        return Math.min(value / 1.2, 100) // WPM
      case "reaction":
      ...
      case "multitasking":
        return Math.min(value / 2, 100) // Score
      default:
        return 50
    }
  }
          ...
          <Card key={testType} className="overflow-hidden">
            <CardContent className="p-6">
              ...
              <div className="space-y-1">
                ...
                <div className={`h-1 mt-1 rounded ${progressColor}`} style={{ width: `${progressValue}%` }}></div>
              </div>
