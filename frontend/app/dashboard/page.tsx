  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (tests.length > 0) {
          const prediction = await getFatiguePrediction()

          if (prediction) {
            setFatigueData({
              score: prediction.fatigue_score || prediction.score || 65,
              level: prediction.fatigue_level || prediction.level || 'Moderate',
              confidence: prediction.confidence || 0.85,
              timestamp: prediction.timestamp ? new Date(prediction.timestamp * 1000).toISOString() : new Date().toISOString(),
            })
          } else {
